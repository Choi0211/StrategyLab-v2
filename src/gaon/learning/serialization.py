...
def repository_from_json(payload: str) -> tuple[tuple[LearningRecord, ...], tuple[KnowledgeClaim, ...], tuple[AuditEvent, ...]]:
    ...
    claims = tuple(KnowledgeClaim.from_dict(claim) for claim in _optional_list(decoded, "claims"))
    audit_events = tuple(AuditEvent.from_dict(event) for event in _require_list(decoded, "audit_events"))
    return records, claims, audit_events
