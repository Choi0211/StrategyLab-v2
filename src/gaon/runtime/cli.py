...
def main(argv: list[str] | None = None) -> int:
    ...
    sub.add_parser("gaon-production-signal-integrity-release-check")
    ...
    sub.add_parser("gaon-production-terminal-validation-retry-boundary-release-check")
    ...
    sub.add_parser("gaon-production-regime-validation-release-check")
    sub.add_parser("gaon-production-parameter-sensitivity-release-check")
    sub.add_parser("gaon-production-transaction-cost-stress-release-check")
    sub.add_parser("gaon-production-monte-carlo-robustness-release-check")
    sub.add_parser("gaon-production-unified-promotion-readiness-release-check")
    sub.add_parser("gaon-production-full-autonomous-quant-research-release-check")
    sub.add_parser("gaon-production-no-fabricated-validation-metrics-release-check")
    sub.add_parser("gaon-production-real-multi-symbol-validation-release-check")
    sub.add_parser("gaon-production-real-oos-validation-release-check")
    sub.add_parser("gaon-production-real-walk-forward-release-check")
    sub.add_parser("gaon-production-real-regime-validation-release-check")
    sub.add_parser("gaon-production-real-parameter-sensitivity-release-check")
    ...
    sub.add_parser("gaon-production-final-autonomous-research-release-check")
    sub.add_parser("gaon-production-final-conversation-release-check")
    sub.add_parser("gaon-production-two-stage-approval-release-check")
    sub.add_parser("gaon-production-candidate-freeze-release-check")
    sub.add_parser("gaon-production-champion-replacement-release-check")
...
def _run(args: argparse.Namespace) -> int:
    if args.command == "config-check":
    ...
    elif args.command == "gaon-production-safe-content-acquisition-release-check":
        from gaon.knowledge.telegram_autonomous_learning import production_safe_content_acquisition_release_check
    ...
    elif args.command == "gaon-production-candidate-multi-symbol-robustness-release-check":
        from gaon.knowledge.research_mission import production_candidate_multi_symbol_robustness_release_check

        payload = production_candidate_multi_symbol_robustness_release_check()
        print(
            "gaon-production-candidate-multi-symbol-robustness-release-check: PASS "
            f"schema_version={payload['schema_version']} "
            f"candidate_primary_identity={str(payload['candidate_primary_identity']).lower()} "
            f"fingerprint_preserved_across_symbols={str(payload['fingerprint_preserved_across_symbols']).lower()} "
            f"symbols_are_evidence_samples={str(payload['symbols_are_evidence_samples']).lower()} "
            f"robustness_status_accumulates={str(payload['robustness_status_accumulates']).lower()} "
            f"oos_state_grounded={str(payload['oos_state_grounded']).lower()} "
            f"walk_forward_state_grounded={str(payload['walk_forward_state_grounded']).lower()} "
            f"cost_stress_state_grounded={str(payload['cost_stress_state_grounded']).lower()} "
            f"regime_state_grounded={str(payload['regime_state_grounded']).lower()} "
            f"restart_persistence={str(payload['restart_persistence']).lower()} "
            f"status_query_read_only={str(payload['status_query_read_only']).lower()} "
    ...
    elif args.command == "gaon-production-canonical-candidate-handoff-release-check":
        from gaon.knowledge.research_mission import production_canonical_candidate_handoff_release_check

        payload = production_canonical_candidate_handoff_release_check()
        print(
            "gaon-production-canonical-candidate-handoff-release-check: PASS "
            f"schema_version={payload['schema_version']} "
            f"report_candidate_canonicalized={str(payload['report_candidate_canonicalized']).lower()} "
            f"canonical_fingerprint_preserved={str(payload['canonical_fingerprint_preserved']).lower()} "
            f"report_label_not_identity={str(payload['report_label_not_identity']).lower()} "
            f"breadth_to_persisted_candidate={str(payload['breadth_to_persisted_candidate']).lower()} "
            f"robustness_uses_same_candidate={str(payload['robustness_uses_same_candidate']).lower()} "
            f"pending_candidate_not_prematurely_rotated={str(payload['pending_candidate_not_prematurely_rotated']).lower()} "
            f"multi_symbol_robustness_accumulates={str(payload['multi_symbol_robustness_accumulates']).lower()} "
            f"restart_mapping_persists={str(payload['restart_mapping_persists']).lower()} "
            f"status_query_read_only={str(payload['status_query_read_only']).lower()} "
    ...
    elif args.command == "gaon-production-research-action-execution-handoff-release-check":
        from gaon.knowledge.research_mission import production_research_action_execution_handoff_release_check

        payload = production_research_action_execution_handoff_release_check()
        details = " ".join(
            f"{key}={str(value).lower() if isinstance(value, bool) else value}"
            for key, value in payload.items()
        )
        print(f"gaon-production-research-action-execution-handoff-release-check: PASS {details}")

    elif args.command == "gaon-production-research-action-persistence-release-check":
        from gaon.knowledge.research_mission import production_research_action_persistence_release_check

        payload = production_research_action_persistence_release_check()
        details = " ".join(
            f"{key}={str(value).lower() if isinstance(value, bool) else value}"
            for key, value in payload.items()
        )
    ...
    elif args.command == "gaon-production-terminal-validation-retry-boundary-release-check":
        from gaon.knowledge.research_mission import production_terminal_validation_retry_boundary_release_check

        payload = production_terminal_validation_retry_boundary_release_check()
        details = " ".join(
            f"{key}={str(value).lower() if isinstance(value, bool) else value}"
            for key, value in payload.items()
        )
        print(f"gaon-production-terminal-validation-retry-boundary-release-check: PASS {details}")
        ...
        payload = production_strategy_space_expansion_release_check()
        details = " ".join(
            f"{key}={str(value).lower() if isinstance(value, bool) else value}"
            for key, value in payload.items()
        )
        print(f"gaon-production-strategy-space-expansion-release-check: PASS {details}")

    elif args.command == "gaon-production-cumulative-sample-persistence-release-check":
        ...
        print(f"gaon-production-cumulative-sample-persistence-release-check: PASS {details}")

    elif args.command == "gaon-production-sample-exhaustion-candidate-decision-release-check":
        ...
        print(f"gaon-production-sample-exhaustion-candidate-decision-release-check: PASS {details}")

    elif args.command == "gaon-production-promotion-target-consistency-release-check":
        from gaon.knowledge.research_mission import production_promotion_target_consistency_release_check

        payload = production_promotion_target_consistency_release_check()
        details = " ".join(
            f"{key}={str(value).lower() if isinstance(value, bool) else value}"
            for key, value in payload.items()
        )
        ...
        print(f"gaon-production-web-api-root-release-check: PASS {details}")
    ...
    elif args.command == "gaon-production-candidate-read-only-routing-release-check":
        from gaon.knowledge.research_mission import production_candidate_read_only_routing_release_check
        ...
        print(f"gaon-production-candidate-read-only-routing-release-check: PASS {details}")

    elif args.command == "gaon-production-typo-tolerant-research-continuation-release-check":
        from gaon.knowledge.research_mission import production_typo_tolerant_research_continuation_release_check
        ...
        print(f"gaon-production-typo-tolerant-research-continuation-release-check: PASS {details}")
        ...
        print(
            ...
            f"cost_mapping_grounded={str(payload['cost_mapping_grounded']).lower()} "
        ...
        print(
            ...
            f"persisted_pre_upgrade_direction_continues={str(payload['persisted_pre_upgrade_direction_continues']).lower()} "
            f"existing_direction_reused={str(payload['existing_direction_reused']).lower()} "
            f"evidence_acquired_from_existing_direction={str(payload['evidence_acquired_from_existing_direction']).lower()} "
            f"direction_not_recreated={str(payload['direction_not_recreated']).lower()} "
            f"one_stage_per_tick={str(payload['one_stage_per_tick']).lower()} "
            f"provider_failure_honest={str(payload['provider_failure_honest']).lower()} "
            f"no_duplicate_evidence={str(payload['no_duplicate_evidence']).lower()} "
        ...
        handlers = {
            ...
            "gaon-production-signal-integrity-release-check": production_signal_integrity_release_check,
            "gaon-production-multi-symbol-validation-release-check": production_multi_symbol_validation_release_check,
            "gaon-production-real-web-news-provider-release-check": production_real_web_news_provider_release_check,
            "gaon-production-real-youtube-provider-release-check": production_real_youtube_provider_release_check,
            "gaon-production-independent-evidence-release-check": production_independent_evidence_release_check,
            "gaon-production-out-of-sample-release-check": production_out_of_sample_release_check,
            "gaon-production-walk-forward-release-check": production_walk_forward_release_check,
            "gaon-production-regime-validation-release-check": production_regime_validation_release_check,
            "gaon-production-parameter-sensitivity-release-check": production_parameter_sensitivity_release_check,
            "gaon-production-transaction-cost-stress-release-check": production_transaction_cost_stress_release_check,
            "gaon-production-monte-carlo-robustness-release-check": production_monte_carlo_robustness_release_check,
            "gaon-production-unified-promotion-readiness-release-check": production_unified_promotion_readiness_release_check,
            "gaon-production-full-autonomous-quant-research-release-check": production_full_autonomous_quant_research_release_check,
            "gaon-production-no-fabricated-validation-metrics-release-check": production_no_fabricated_validation_metrics_release_check,
            "gaon-production-real-multi-symbol-validation-release-check": production_real_multi_symbol_validation_release_check,
            "gaon-production-real-oos-validation-release-check": production_real_oos_validation_release_check,
            "gaon-production-real-walk-forward-release-check": production_real_walk_forward_release_check,
            "gaon-production-real-regime-validation-release-check": production_real_regime_validation_release_check,
            "gaon-production-real-parameter-sensitivity-release-check": production_real_parameter_sensitivity_release_check,
            ...
            "gaon-production-final-autonomous-research-release-check": production_final_autonomous_research_release_check,
            "gaon-production-final-conversation-release-check": production_final_conversation_release_check,
            "gaon-production-two-stage-approval-release-check": production_two_stage_approval_release_check,
            "gaon-production-candidate-freeze-release-check": production_candidate_freeze_release_check,
            "gaon-production-champion-replacement-release-check": production_champion_replacement_release_check,
