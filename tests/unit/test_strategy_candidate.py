...
class StagnationRotationTests(unittest.TestCase):
    ...
    def test_strategy_hypothesis_space_exhausted_only_after_all_rounds_and_paradigm_rotation(self) -> None:
        # feature/autonomous-paradigm-rotation + Priority 6: after both
        # breakout expansion rounds, expand_strategy_space_candidate
        # rotates through NON_BREAKOUT_STRATEGY_FAMILY_TEMPLATES then
        # REGIME_FILTERED_STRATEGY_FAMILY_TEMPLATES (deterministic order),
        # one candidate at a time. Relative strength is NOT reachable
        # without a multi-symbol context, so with the default flag the
        # terminal ``strategy_hypothesis_space_exhausted`` fires once the
        # non-breakout + regime families are used and carries
        # ``relative_strength_requires_multi_symbol_context``.
        from gaon.knowledge.strategy_candidate import (
        ...
        rotation_families = [
            ...
            *(t.family for t in REGIME_FILTERED_STRATEGY_FAMILY_TEMPLATES),
        ]
        candidates = list(
            new_candidate(template.family, sequence=index + 1, now=NOW)
